...
public class TSPDisjointCycle {
    ...
    private void dfs(int current, Map<Integer, List<Integer>> adjacencyList, boolean[] visited, List<Integer> cycle) {
        visited[current] = true;
        cycle.add(current);

        for (int neighbor : adjacencyList.get(current)) {
            if (!visited[neighbor]) {
                dfs(neighbor, adjacencyList, visited, cycle);
            }
        }
    }
    private List<Integer> combineCyclesIntoTour(List<List<Integer>> cycles) {
    ...
    private List<List<Integer>> findDisjointCycles(List<int[]> multigraph, int numberOfCities) {
        ...
        for (int i = 0; i < numberOfCities; i++) {
            if (!visited[i]) {
                ...
                dfs(i, adjacencyList, visited, cycle);
                cycles.add(cycle);
            }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        List<Integer> tour = tsp.christofidesWithDisjointCycle(costMatrix);
        ...
        System.out.println();
        System.out.println("Minimum cost: " + minCost / 10.0);
    }
    public List<Integer> christofidesWithDisjointCycle(int[][] costMatrix) {
        ...
        List<List<Integer>> cycles = findDisjointCycles(multigraph, numberOfCities);

        return combineCyclesIntoTour(cycles);
    }
}
